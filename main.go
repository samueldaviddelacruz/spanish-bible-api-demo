...
func main() {
	...
	huma.Register(api, huma.Operation{
		...
		Tags:        []string{"Books"},
	}, func(ctx context.Context, i *struct{}) (*BooksResponse, error) {
		books := []Book{}
		chapters := []Chapter{}
	...
	huma.Register(api, huma.Operation{
		...
		Tags:        []string{"Book"},
	}, func(ctx context.Context, input *BookRequest) (*BookResponse, error) {
		book := Book{}

		err := db.Get(&book, `SELECT id, name, "order", testament FROM books WHERE id = ?`, input.Id)
		if err != nil {
			if err != sql.ErrNoRows {
				return nil, fmt.Errorf("Error while getting book from DB: %v", err)
			}
			return nil, huma.Error404NotFound(fmt.Sprintf("Book not found: %s", input.Id))
		}
